...
function UploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const [annotationType, setAnnotationType] = useState('yolo')
  ...
  const annotationTypes = ['yolo', 'voc', 'coco']
  ...
  // 校验名称是否已存在
  async function validateName(datasetName) {
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={styles.section}>
          ...
          <select value={annotationType} onChange={e => setAnnotationType(e.target.value)} style={styles.select}>
