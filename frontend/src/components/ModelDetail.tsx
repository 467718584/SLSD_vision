...
// 图表项组件
function ChartItem({ title, src, onPreview }: { title: string; src?: string; onPreview: (src: string) => void }) {
  if (!src) {
    return (
...
function ModelDetail({ model, datasets, onBack, onEdit }: ModelDetailProps) {
  ...
  // 精度颜色
  const accuracyColor = (model.accuracy || 0) >= 95 ? C.success : (model.accuracy || 0) >= 85 ? C.primary : C.warning
          ...
          <InfoItem label="模型精度" value={
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '60px', height: '6px', background: C.gray6, borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{ width: `${model.accuracy || 0}%`, height: '100%', background: accuracyColor, borderRadius: '3px' }} />
              </div>
